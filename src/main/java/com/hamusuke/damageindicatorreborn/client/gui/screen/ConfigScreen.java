...
public class ConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        this.list.addButton(new Button(0, 0, 0, 20, Component.translatable(MOD_ID + ".config.colorConfig.title"), p_onPress_1_ -> this.minecraft.setScreen(new ColorSettingsScreen(this))));

        this.addRenderableWidget(new Button(0, this.height - 20, this.width / 2, 20, CommonComponents.GUI_CANCEL, p_onPress_1_ -> this.onClose()));
        this.addRenderableWidget(new Button(this.width / 2, this.height - 20, this.width / 2, 20, CommonComponents.GUI_DONE, p_onPress_1_ -> {
            Config.CONFIG.save();
            this.onClose();
        }));
    }
    ...
    private static class ColorSettingsScreen extends Screen {
        ...
        private class ColorList extends ObjectSelectionList<ColorList.Color> {
            ...
            private class Color extends ObjectSelectionList.Entry<Color> {
                ...
                private Color(RGBValue rgbConfig) {
                    this.button = new Button(ColorSettingsScreen.this.width / 4, 0, ColorSettingsScreen.this.width / 2, 20, Component.translatable(MOD_ID + ".config.color." + rgbConfig.path), p_onPress_1_ -> ColorSettingsScreen.this.minecraft.setScreen(new ColorMixingScreen(ColorSettingsScreen.this, rgbConfig)));
                }
        ...
        private static class ColorMixingScreen extends Screen {
            ...
            public void render(PoseStack p_96562_, int p_96563_, int p_96564_, float p_96565_) {
                ...
                int color = ARGB32.color(255, this.red.getValueInt(), this.green.getValueInt(), this.blue.getValueInt());
                this.fillGradient(p_96562_, this.width / 4, this.height / 2 + 5, this.width * 3 / 4, this.height - 25, color, color);
            }
